...
from .common import MyProperties
...
class MyProcessor(ProcessorInterface):
    ...
    def finish(self):
        ...
        if self.request:
            ...
            response.set_attribute("msg_pool", self.request.get_attribute("msg_pool"))  # 带IP端口
            response.set_attribute("processor_name", self.class_name)
            print("finish.method._finished = %s, 输入请求为：" % self._finished, self.request)
    ...
    def on_finish(self):
        ...
        if self.response:
            ...
            file_path = path.Path(self.response.get_attribute("msg_pool")['todo']).joinpath(sig)
            print("向外发送通知消息:", file_path, " 消息的内容为:", self.response)
            self._finished = False
